...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/music:/music \
	# -v $(CURDIR)/data:/var/lib/mpd \
	# -v $(CURDIR)/root/defaults/asound.conf:/etc/asound.conf \
	# -v $(CURDIR)/root/defaults/mpd.conf:/defaults/mpd.conf \
...
OTHERFLAGS := \
	...
	--cap-add=SYS_NICE \
	# --privileged \
	# -e MPD_CONF=/etc/mpd.conf \
	# -e MPD_HOME=/var/lib/mpd \
	# -e MPD_LOGS=/var/logs/mpd \
	# -e MPD_GROUP=audio \
	# -e TZ=Asia/Kolkata \
	# \
...
player : ## start ncmpcpp in container
	docker exec -it $(CNTNAME) ncmpcpp
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/mpd.conf \
		-SL https://raw.githubusercontent.com/MusicPlayerDaemon/MPD/master/doc/mpdconf.example \
	#
...
$(shell docker run --rm --pull=never \
	...
	--version \
	2>/dev/null \
	| grep '^Music Player Daemon ' | awk '{print $$4}')
endef
# -- }}}
